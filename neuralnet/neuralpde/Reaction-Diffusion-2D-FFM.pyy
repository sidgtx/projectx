...
def neural_network(x):
    ...
    net = dde.nn.FNN([x.shape[1]] + [128] * 4 + [2], activation="tanh", kernel_initializer="Glorot uniform")
    return net(x)
...
bc_u = dde.icbc.NeumannBC(geomtime, lambda x: 0, lambda x, on_boundary: on_boundary, component=0)
bc_v = dde.icbc.NeumannBC(geomtime, lambda x: 0, lambda x, on_boundary: on_boundary, component=1)
...
# Define the model
net = dde.nn.FNN([3] + [128] * 4 + [2], activation="tanh", kernel_initializer="Glorot uniform")
