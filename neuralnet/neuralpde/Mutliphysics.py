...
def pde_system(x, y):
    ...
    fx = lam * tf.gradients(div_v, x)[0][:, 0:1] + 2 * mu * tf.gradients(eps_xx, x)[0][:, 0:1]
    fy = lam * tf.gradients(div_v, x)[0][:, 1:2] + 2 * mu * tf.gradients(eps_yy, x)[0][:, 1:2]
...
bc_u = dde.DirichletBC(geomtime, lambda x: 0.0, lambda _, on_boundary: on_boundary, component=0)
bc_phi = dde.DirichletBC(geomtime, lambda x: 0.0, lambda _, on_boundary: on_boundary, component=1)
bc_vx = dde.DirichletBC(geomtime, lambda x: 0.0, lambda _, on_boundary: on_boundary, component=2)
bc_vy = dde.DirichletBC(geomtime, lambda x: 0.0, lambda _, on_boundary: on_boundary, component=3)
...
X, Y = np.meshgrid(x, y)
XYT = np.hstack((X.flatten()[:, None], Y.flatten()[:, None], t_fixed * np.ones((nx * ny, 1))))
...
plt.streamplot(x, y, vx_pred.T, vy_pred.T, color=np.sqrt(vx_pred.T**2 + vy_pred.T**2), cmap="plasma", density=1)
