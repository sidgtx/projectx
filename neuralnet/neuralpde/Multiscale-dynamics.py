...
def pde(x, y):
    ...
    u_xx = dde.grad.hessian(y, x, component=0, i=0, j=0) + dde.grad.hessian(y, x, component=0, i=1, j=1)
    c_xx = dde.grad.hessian(y, x, component=1, i=0, j=0) + dde.grad.hessian(y, x, component=1, i=1, j=1)

    mu = mu0 * c / (K + c)
    return [
        u_t - Du * u_xx - mu * u * (1 - u),
        c_t - eps * c_xx + kappa * u * c
    ]
...
bc1 = dde.NeumannBC(geomtime, lambda x: 0, lambda _, on_boundary: on_boundary, component=0)
bc2 = dde.NeumannBC(geomtime, lambda x: 0, lambda _, on_boundary: on_boundary, component=1)

# Data definition
data = dde.data.TimePDE(
    geomtime, pde, [ic1, ic2, bc1, bc2],
    num_domain=5000,
    num_boundary=1000,
    num_initial=500,
    solution=None,
